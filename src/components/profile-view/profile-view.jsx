...
export class ProfileView extends React.Component {
  ...
  getUserDetails(token) {
    axios.get(`https://myflixapp1.herokuapp.com/users/${this.props.user}`, {
...
updateUserDetails(e) {
    ...
    if (form.checkValidity() === false) {
        ...
        axios.put(`https://myflixapp1.herokuapp.com/users/${user}`, {
...
deleteUserDetails() {
    let token = localStorage.getItem('token');
    let user = localStorage.getItem('user');
    axios.delete(`https://myflixapp1.herokuapp.com/users/${user}`, {
        headers: { Authorization: `Bearer ${token}`}
    }).then(response => {
        const data = response.data;
    ...
    return (
        <div className="profile_view">
            ...
            <Card bg="secondary" text="light" border="light">
                <Card.Body>
                    ...
                    {this.state.Birthdate && (
                        <Card.Text><span className="profile_heading">Date of Birth: </span>{Intl.DateTimeFormat().format(new Date(this.state.Birthdate))}</Card.Text>
                    )}
                </Card.Body>
            </Card>
            ...
            <Card bg="secondary" text="light" border="light">
                <Card.Body>
                    ...
                    <Form noValidate validated={this.state.validated}>
                        ...
                        <Button variant="dark" style={{ color: "white" }} type="submit" onClick={this.updateUserDetails}>
                            Update Details
                        </Button>
                        {/* Button to go back to the previous view */}
                        <Button onClick={() => onBackClick(null)} variant="dark" style={{ color: "white" }}>Back</Button>
                        {/* Button for deleting the user. This will first open the Modal defined above */}
                        <Button className="float-right" variant="dark" style={{ color: "white" }} onClick={this.showModal}>
                            Delete User Profile
                        </Button>
                    </Form>
                </Card.Body>
            ...
            <Card bg="secondary" text="light" border="light" align="center" style={{ color: "white" }}>
                <Card.Title>{this.state.Username}'s Favorites:</Card.Title>
            <Row>
