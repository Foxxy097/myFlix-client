...
export class MovieView extends React.Component {
    ...
    addFavorite() {
        ...
        axios.post(`https://myflixapp1.herokuapp.com/users/${this.props.user}/movies/${this.props.movie._id}`, {}, {
            headers: { Authorization: `Bearer ${token}` }
        }).then(response => {
    ...
    removeFavorite() {
        let token = localStorage.getItem('token');
        axios.delete(`https://myflixapp1.herokuapp.com/users/${this.props.user}/movies/${this.props.movie._id}`, {
            headers: { Authorization: `Bearer ${token}` }
        }).then(response => {
                ...
                <Card.Body>
                    <Row>
                        ...
                        <Col xs={12} md={6}>
                            ...
                            {isFavoriteNew ? (
                                <Button className="float-right" variant="dark" style={{ color: "white" }} onClick={this.removeFavorite}>
                                    Remove from Favorites
                                </Button>
                            ) : (
                                <Button className="float-right" variant="dark" style={{ color: "white" }} onClick={this.addFavorite}>
                                    Add to Favorites
                                </Button>
                            )}
